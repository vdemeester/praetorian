...
func setup(c *cli.Context) {
	if len(c.Args()) != 2 {
		fmt.Fprintf(os.Stderr, "setup commands needs two arguments : praetorian setup user alias\n")
		os.Exit(1)
	}
	...
	sshMagicString := fmt.Sprintf(`%s command="FINGERPRINT=%s NAME=%s praetorian exec $SSH_ORIGINAL_COMMAND",no-X11-forwarding`, finalKey, keyFingerPrint, alias)
